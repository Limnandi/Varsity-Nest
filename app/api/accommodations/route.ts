...
export async function GET(request: NextRequest) {
  try {
    ...
    let accommodations
    if (providerId) {
      accommodations = await fetchAccommodationsByProvider(providerId, limit ? parseInt(limit) : 50)
    } else if (status) {
      accommodations = await fetchAccommodationsByStatus(status, limit ? parseInt(limit) : 50, offset ? parseInt(offset) : 0)
    } else if (featured === 'true') {
      const { fetchFeaturedAccommodations } = await import('@/lib/repos/accommodations')
      accommodations = await fetchFeaturedAccommodations(limit ? parseInt(limit) : 50)
    } else {
      accommodations = await fetchAccommodationsByStatus('accredited', limit ? parseInt(limit) : 50, offset ? parseInt(offset) : 0)
    }
