...
interface AgentBillingResponse {
  agent: AgentData
  invoices: Invoice[]
...
export default function AgentBilling() {
  ...
  const [subscriptionSummary, setSubscriptionSummary] = useState<AgentBillingResponse["subscriptionSummary"] | null>(null)
  ...
  const handlePlanSelect = useCallback((planId: SubscriptionPlanId) => {
    router.push(`/agent/billing/payment?planId=${planId}`)
  }, [router])
  ...
  const fetchAgentData = useCallback(async () => {
    try {
      ...
      const data: AgentBillingResponse = await response.json()
      setAgent(data.agent)
  ...
  const handleOpenManagementLink = async () => {
    // Use subscriptionToken from agent, or subscription code from subscriptionDetails
    let subscriptionCode = agent?.subscriptionToken || subscriptionDetails?.token
    
    // If no subscription code but agent is in trial, try to find it
    if (!subscriptionCode && (agent?.billingInfo.isInTrial || agent?.billingInfo.subscriptionStatus === 'trial')) {
      try {
        ...
        if (findResponse.ok) {
          ...
          if (findData.success && findData.subscriptionCode) {
            ...
            // Update agent state with found subscription code
            if (agent && subscriptionCode) {
              setAgent({ ...agent, subscriptionToken: subscriptionCode || null })
            }
  ...
  const currentPlanId = agent?.billingInfo.planId ?? null
  ...
  const planLimit = agent?.billingInfo.planLimit ?? currentPlan?.maxProperties ?? null
  const publishedCount = agent?.billingInfo.publishedCount ?? subscriptionSummary?.publishedCount ?? 0
  const totalProperties = agent?.billingInfo.totalProperties ?? subscriptionSummary?.totalCount ?? publishedCount
  ...
  const hasActiveSubscription = agent.billingInfo.subscriptionStatus === 'active'
  const pendingAmount = hasActiveSubscription ? 0 : agent.billingInfo.planPrice
    ...
    <AuthGuard requiredRole="agent">
      <DashboardLayout userRole="agent">
          ...
          <div className="relative border border-white/10 bg-black/20 backdrop-blur-xl rounded-2xl p-4 sm:p-6 lg:p-8 text-white shadow-2xl shadow-blue-500/20">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div className="min-w-0 flex-1">
                ...
                <p className="text-neutral-300 text-sm sm:text-lg break-words">{agent.businessName}</p>
              </div>
              <button
                onClick={handleRefresh}
            ...
            <div className="group relative border border-white/10 bg-black/20 backdrop-blur-xl rounded-2xl p-4 sm:p-6 lg:p-8 text-white shadow-2xl shadow-purple-500/10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
                {plans.map((plan) => {
                  ...
                  const hasActiveSubscription = agent.billingInfo.subscriptionStatus === 'active' || agent.billingInfo.subscriptionStatus === 'trial'
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
            ...
            <div className="group relative border border-white/10 bg-gradient-to-br from-blue-500/20 to-blue-600/20 backdrop-blur-xl rounded-2xl p-6 text-white shadow-2xl shadow-blue-500/20 hover:shadow-blue-500/30 transition-all duration-300 hover:scale-[1.02]">
              ...
              <div className="space-y-2">
                {agent.billingInfo.isInTrial && agent.billingInfo.trialEndDate ? (
                  <>
                    ...
                    <div className="mt-3 pt-3 border-t border-white/10">
                      <p className="text-xs text-neutral-400">
                        After trial ends, monthly subscription: R{agent.billingInfo.planPrice.toFixed(2)}/month
                      </p>
                  ...
                  <>
                    ...
                    <p className="text-4xl font-bold">
                      R{agent.billingInfo.planPrice.toFixed(2)}
              ...
              {!agent.billingInfo.isInTrial && !agent.billingInfo.isFirstTimeUser && (
                <button 
                  onClick={() => {
                    setIsNavigatingToPayment(true)
                    router.push("/agent/billing/payment")
                  }}
        ...
        {subscriptionSummary && (
            ...
            <div className="group relative border border-white/10 bg-black/20 backdrop-blur-xl rounded-2xl p-6 text-white shadow-2xl shadow-blue-500/10">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-white">{agent.billingInfo.planName}</p>
                </div>
              ...
              <p className="text-sm text-neutral-400 mt-2">{agent.billingInfo.planDescription}</p>
          ...
          {(agent.subscriptionToken || subscriptionDetails || agent.billingInfo.isInTrial || agent.billingInfo.subscriptionStatus === 'trial') && subscriptionSummary && (
            <div className="group relative border border-white/10 bg-black/20 backdrop-blur-xl rounded-2xl p-4 sm:p-6 lg:p-8 text-white shadow-2xl shadow-blue-500/10">
              ...
              {isLoadingSubscription ? (
                ...
                <div className="space-y-6">
                  ...
                  {(agent.subscriptionToken || agent.billingInfo.isInTrial || agent.billingInfo.subscriptionStatus === 'trial') && (
                    <div className="flex flex-wrap gap-3">
                      <button
                        onClick={handleOpenManagementLink}
                        disabled={isGeneratingManagementLink || isManagingSubscription}
