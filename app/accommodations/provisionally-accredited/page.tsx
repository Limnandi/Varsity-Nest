...
export default function ProvisionallyAccreditedAccommodations() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        let accommodations = CacheManager.getCachedAccommodationsByStatusClient('provisionally-accredited', 100, 0)
        
        if (!accommodations || !Array.isArray(accommodations)) {
          // Fetch from existing API endpoint
          const response = await fetch('/api/accommodations?status=provisionally-accredited&limit=100&offset=0')
          ...
          if (accommodations && accommodations.length > 0) {
            CacheManager.cacheAccommodationsByStatusClient('provisionally-accredited', 100, 0, accommodations)
          }
        }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-16">
          <h1 className="text-5xl font-bold text-white mb-6 drop-shadow-2xl tracking-tight">
            Provisionally-Accredited Accommodations
          </h1>
          <p className="text-xl text-neutral-300 drop-shadow-lg mb-8 max-w-3xl mx-auto leading-relaxed">
            Explore our selection of provisionally accredited student accommodations in Bloemfontein. These properties
            are in the process of meeting full accreditation standards and offer quality student living options.
          </p>
        ...
        <div className="relative border border-white/10 bg-black/20 backdrop-blur-xl rounded-2xl p-6 mb-8 text-white shadow-2xl shadow-orange-500/10">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div className="flex items-center space-x-2">
              ...
              <select
                ...
                className="px-3 py-2 bg-black/20 border border-white/10 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent text-white placeholder-neutral-300 backdrop-blur-sm"
              >
          ...
          <div className="text-center py-12">
            <div className="relative border border-white/10 bg-black/20 backdrop-blur-xl rounded-2xl p-8 text-white shadow-2xl shadow-orange-500/10">
              ...
              <button
                ...
                className="group relative inline-flex items-center justify-center px-8 py-4 text-lg font-semibold text-white transition-all duration-300 ease-in-out bg-gradient-to-r from-orange-600 to-red-600 rounded-xl shadow-lg shadow-orange-500/20 hover:scale-[1.02] hover:shadow-orange-500/40 active:scale-[0.98]"
              >
                <span className="relative z-10">Reset Search</span>
                <div className="absolute inset-0 rounded-xl bg-gradient-to-r from-orange-600 to-red-600 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
              </button>
            </div>
          </div>
        )}
